...
def SE_SRBD(NumBSAnt, NumRISEle, NumUE, NumPUE, NumSUE, RISgroup, Bandwidth, sigma2, L_spreadingfactor,
              H_U2B_Ric, H_R2B_Ric, H_U2R_Ric, S_U2B_Ric, S_U2R_Ric, BB_power_ini, BB_theta_ini, BB_onoff_ini, P_Rmin, S_Rmin):
    ...
    U_BD_B_Channel = np.zeros([RISgroup, NumSUE, NumBSAnt], dtype = 'complex_')
    print(H_U2R_Ric.shape)
    print(H_R2B_Ric.shape)
    U_BD_B_Channel[0] = H_U2R_Ric @ H_R2B_Ric
    U_BD_B_Channel[1] = S_U2R_Ric @ H_R2B_Ric
    # U_BD_B_Channel_real = U_BD_B_Channel.real
    # U_BD_B_Channel_imag = U_BD_B_Channel.imag
    ...
    def obj_function(model, L_spreadingfactor, PT_static, RIS_static, SIC_dissapation, PUE_static, SUE_static):
        ...
        return ( BD_throughput_PUE(model) + BD_throughput_SUE(model, L_spreadingfactor) ) / BD_power_consumption(model, PT_static, RIS_static, SIC_dissapation, PUE_static, SUE_static)
        
        
    def obj_function_SE(model, L_spreadingfactor):
        
        # return sum((Bandwidth*(log(1+((SUE_power[idx_UE_obj] * RIS_square(model, 1, idx_UE_obj)) / (interference(model, idx_UE_obj) + sigma2))) / log(2) ))/(SUE_power[idx_UE_obj]*mu +P_k) for idx_UE_obj in range(NumSUE))
        # return sum((Bandwidth*(log(1+((model.power[idx_UE_obj] * RIS_square(model, 1, idx_UE_obj)) / (interference(model, idx_UE_obj) + sigma2))) / log(2) ))/(model.power[idx_UE_obj]*mu +P_k) for idx_UE_obj in range(NumSUE))
        # return sum((0.5*((log(1 + ((model.power[idx_UE_obj] * Channel_square(model, 1, idx_UE_obj)) / sigma2)) / log(2) ) + \
        #             (log(1 + ((model.power[idx_UE_obj] * Channel_square_PUE(model, 1, idx_UE_obj)) / sigma2)) / log(2) )) )/(model.power[idx_UE_obj]*mu + P_k) for idx_UE_obj in range(NumSUE))
        return ( BD_throughput_PUE(model) + BD_throughput_SUE(model, L_spreadingfactor) )
    ...
    def BD_channel_square(model, idx_group, idx_square_UE):
        
        return sum((H_U2B_RicRe[idx_group][idx_square_UE][idx_BS].real + phase_shift_discrete(model) * U_BD_B_Channel[idx_group][idx_square_UE][idx_BS].real)**2 +\
                    (H_U2B_RicRe[idx_group][idx_square_UE][idx_BS].imag + phase_shift_discrete(model) * U_BD_B_Channel[idx_group][idx_square_UE][idx_BS].imag)**2 for idx_BS in range(0, NumBSAnt))
    
    
    def BD_channel_square_PUE(model, idx_group, idx_square_UE):
        
        return sum((H_U2B_RicRe[idx_group][idx_square_UE][idx_BS].real - phase_shift_discrete(model) * U_BD_B_Channel[idx_group][idx_square_UE][idx_BS].real)**2 +\
                    (H_U2B_RicRe[idx_group][idx_square_UE][idx_BS].imag - phase_shift_discrete(model) * U_BD_B_Channel[idx_group][idx_square_UE][idx_BS].imag)**2 for idx_BS in range(0, NumBSAnt))
    
    def BD_throughput_PUE(model):
        ...
        return sum((0.5*((log(1 + (( power_transform(model, idx_PUE_obj) * BD_channel_square(model, 0, idx_PUE_obj)) / sigma2)) / log(2) ) + \
                    (log(1 + ((power_transform(model, idx_PUE_obj) * BD_channel_square_PUE(model, 0, idx_PUE_obj)) / sigma2)) / log(2) )) ) for idx_PUE_obj in range(NumPUE))
    
    def BD_throughput_SUE(model, L_spreadingfactor):
        ...
        return sum(( (1/L_spreadingfactor) * (log(1 + ((L_spreadingfactor * power_transform(model, idx_SUE_obj) * BD_channel_square(model, 1, idx_SUE_obj)) / sigma2)) / log(2) ) ) for idx_SUE_obj in range(NumSUE))
    
    def BD_power_consumption(model, PT_static, RIS_static, SIC_dissapation, PUE_static, SUE_static):
    ...
    def constraints_04(model, idx_qos_p):
        ...
        return (0.5*((log(1 + ((power_transform(model, idx_qos_p) * BD_channel_square(model, 0, idx_qos_p)) / sigma2)) / log(2) ) + \
                    (log(1 + ((power_transform(model, idx_qos_p) * BD_channel_square_PUE(model, 0, idx_qos_p)) / sigma2)) / log(2) )) ) >= P_Rmin
    
    def constraints_05(model, idx_qos_s, L_spreadingfactor):
        
        # return t_b*Bandwidth*(log(1+((power_quantized(model, idx_cons) * RIS_square(model, idx_cons)) /sigma2)) / log(2)) >= LocalDataSize/T_e
        # return Bandwidth * (log(1+((PUE_power[idx_pqos] * RIS_square_PUE(model, 0, idx_pqos)) /(interference_PUE(model, idx_pqos) + sigma2))) / log(2)) >= Bandwidth * (log(sqrt(2))/log(2)) 
        return ( (1/L_spreadingfactor) * (log(1 + ((L_spreadingfactor * power_transform(model, idx_qos_s) * BD_channel_square(model, 1, idx_qos_s)) / sigma2)) / log(2) ) ) >= S_Rmin 
    ...
    model.reflect = Var([i for i in range(NumRISEle)], bounds=(0, 8), within=Integers, initialize = BB_theta_ini)
    ...
    model.obj = Objective(expr=obj_function_SE(model, L_spreadingfactor), sense=maximize)  #-----------------setting objective function-------------------#
    ...
    opt.options['bonmin.time_limit'] = 0.0005
    ...
    SymbioticBD_BB_status2 = "optimal"
    ...
    model_theta_BB = np.array(list(model.reflect.get_values().values()))
    model_theta_BB = model_theta_BB * (1/8)
    ...
    SymbioticBD_BB_status = results.solver.termination_condition
    SymbioticBD_BB_status = SymbioticBD_BB_status2
    SymbioticBD_BB_objective = model.obj()
    ...
    RIS_matrix_multiply_P = model_theta_BB * H_U2R_Ric @ H_R2B_Ric
    RIS_matrix_multiply_S = model_theta_BB * S_U2R_Ric @ H_R2B_Ric
    ...
    Throughput_S_sum = sum( (1/L_spreadingfactor) * ( ma.log2(1 +  (L_spreadingfactor * model_power_BB[idx_sum_s] * Diag_square_S[idx_sum_s] / sigma2) ) ) 
    ...
    print("DQN BB status: ", SymbioticBD_BB_status)
    print("Power consume : ", sum_power, ":W")
    print("Solver Obj SEBD: ", SymbioticBD_BB_objective)
    # print("*************************************************************************")
    ...
    print("obj veri SEBD: ", Obj_veri_SE)
    # print("Interference PUE: ", Interference_PUE)
    Symbiotic_BB_time = results.solver.time
    
    
    # return 0
    return SymbioticBD_BB_status, SymbioticBD_BB_objective, model_power_BB, model_theta_BB, Symbiotic_BB_time
