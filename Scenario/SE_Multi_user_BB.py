...
def MultiU_BB_SE(NumBSAnt, NumRISEle, NumUE, NumPUE, NumSUE, RISgroup, Bandwidth, sigma2, L_spreadingfactor,
    ...
    def obj_function_SE(model, L_spreadingfactor):
        ...
        # return sum((0.5*((log(1 + ((model.power[idx_UE_obj] * Channel_square(model, 1, idx_UE_obj)) / sigma2)) / log(2) ) + \
        #             (log(1 + ((model.power[idx_UE_obj] * Channel_square_PUE(model, 1, idx_UE_obj)) / sigma2)) / log(2) )) )/(model.power[idx_UE_obj]*mu + P_k) for idx_UE_obj in range(NumSUE))
        return ( throughput_PUE(model) + throughput_SUE(model, L_spreadingfactor) )
    ...
    def throughput_SUE(model, L_spreadingfactor):
        ...
        return sum(( (1/L_spreadingfactor) * (log(1 + ((L_spreadingfactor * power_transform(model, idx_SUE_obj) * Channel_square(model, 1, idx_SUE_obj)) / sigma2)) / log(2) ) ) for idx_SUE_obj in range(NumSUE))
    ...
    def constraints_05(model, idx_qos_s, L_spreadingfactor):
        ...
        # return t_b*Bandwidth*(log(1+((power_quantized(model, idx_cons) * RIS_square(model, idx_cons)) /sigma2)) / log(2)) >= LocalDataSize/T_e
        # return Bandwidth * (log(1+((PUE_power[idx_pqos] * RIS_square_PUE(model, 0, idx_pqos)) /(interference_PUE(model, idx_pqos) + sigma2))) / log(2)) >= Bandwidth * (log(sqrt(2))/log(2)) 
        return ( (1/L_spreadingfactor) * (log(1 + ((L_spreadingfactor * power_transform(model, idx_qos_s) * Channel_square(model, 1, idx_qos_s)) / sigma2)) / log(2) ) ) >= S_Rmin 
    ...
    print("Solver Obj SESR: ", Symbiotic_BB_objective)
    # print("*************************************************************************")
    ...
    print("obj veri SESR: ", Obj_veri_SE)
