...
def SE_RIS_I(NumBSAnt, NumRISEle, NumUE, NumPUE, NumSUE, RISgroup, Bandwidth, sigma2,
              H_U2B_Ric, H_R2B_Ric, H_U2R_Ric, S_U2B_Ric, S_U2R_Ric, BB_power_ini, BB_power_iniS, BB_theta_ini, BB_onoff_ini, P_Rmin, S_Rmin):
    ...
    T_Conbime = np.zeros([RISgroup, NumBSAnt*NumPUE, NumRISEle], dtype = 'complex_')
    ...
    # print(H_U2B_Ric)
    H_U2B_RicRe[0] = H_U2B_Ric
    # print(S_U2B_Ric.shape)
    ...
    flag_row_1 = 0
    flag_row_2 = 0
    for idx_B2R_i in range(0, NumBSAnt*int(NumUE/2)):
        # idx_R2B_j = 0
    ...
    def obj_function(model, PT_static, RIS_static, PUE_static, SUE_static):
        ...
        return ( throughput_PUE(model) + throughput_SUE(model) ) / power_consumption(model, PT_static, RIS_static, PUE_static, SUE_static)
        
    
    def obj_function_SE(model):
        
        # return sum((Bandwidth*(log(1+((SUE_power[idx_UE_obj] * RIS_square(model, 1, idx_UE_obj)) / (interference(model, idx_UE_obj) + sigma2))) / log(2) ))/(SUE_power[idx_UE_obj]*mu +P_k) for idx_UE_obj in range(NumSUE))
        # return sum((Bandwidth*(log(1+((model.power[idx_UE_obj] * RIS_square(model, 1, idx_UE_obj)) / (interference(model, idx_UE_obj) + sigma2))) / log(2) ))/(model.power[idx_UE_obj]*mu +P_k) for idx_UE_obj in range(NumSUE))
        # return sum((0.5*((log(1 + ((model.power[idx_UE_obj] * Channel_square(model, 1, idx_UE_obj)) / sigma2)) / log(2) ) + \
        #             (log(1 + ((model.power[idx_UE_obj] * Channel_square_PUE(model, 1, idx_UE_obj)) / sigma2)) / log(2) )) )/(model.power[idx_UE_obj]*mu + P_k) for idx_UE_obj in range(NumSUE))
        return ( throughput_PUE(model) + throughput_SUE(model) ) 
    ...
    def throughput_PUE(model):
        ...
        return sum((log(1 + (( power_transform(model, idx_PUE_obj) * Channel_square(model, 0, idx_PUE_obj)) /
                             throughput_PUE_interference(model, idx_PUE_obj))) / log(2) ) for idx_PUE_obj in range(NumPUE))
    
    def throughput_PUE_interference(model, idx_PUE_thr):
        
        # return sum(RIS_square(model, 1, idx_SUE)*PUE_power[idx_pu] for idx_pu in range(NumPUE)) + RIS_square(model, 1, idx_SUE)*SUE_power[1 - idx_SUE]
        # return sum(RIS_square_PUE(model, 1, idx_SUE)*PUE_power[idx_pu] for idx_pu in range(NumPUE)) + RIS_square(model, 1, idx_SUE)*model.power[1 - idx_SUE]
        return (sum(interference_index(model, idx_PUE_thr, idx_PUE_pir) * power_transform(model, idx_PUE_pir) * Channel_square(model, 0, idx_PUE_thr) for idx_PUE_pir in range(NumPUE)) + \
            sum(power_transformS(model, idx_PUE_sir) * Channel_square(model, 0, idx_PUE_thr) for idx_PUE_sir in range(NumSUE)) + sigma2)
    
    def throughput_SUE(model):
        ...
        return sum(( (log(1 + ((power_transformS(model, idx_SUE_obj) * Channel_square(model, 1, idx_SUE_obj)) / 
                               throughput_SUE_interference(model, idx_SUE_obj) )) / log(2) ) ) for idx_SUE_obj in range(NumSUE))
    
    def throughput_SUE_interference(model, idx_SUE_thr):
        
        # return sum(RIS_square_PUE(model, 0, idx_PUEI)*model.power[idx_su] for idx_su in range(NumSUE)) + RIS_square_PUE(model, 0, idx_PUEI)*PUE_power[1 - idx_PUEI]
        # return sum(RIS_square(model, 0, idx_PUEI)*model.power[idx_su] for idx_su in range(NumSUE)) + RIS_square_PUE(model, 0, idx_PUEI)*PUE_power[1 - idx_PUEI]
        return  (sum(power_transform(model, idx_SUE_pir) * Channel_square(model, 1, idx_SUE_thr) for idx_SUE_pir in range(NumPUE))+ \
            sum((interference_index(model, idx_SUE_thr, idx_SUE_sir) * power_transformS(model, idx_SUE_sir) * Channel_square(model, 1, idx_SUE_thr) )  for idx_SUE_sir in range(NumSUE)) + sigma2)
    
    
    def power_consumption(model, PT_static, RIS_static, PUE_static, SUE_static):
        
        
        return sum((power_transform(model, idx_UE_obj)*mu + PUE_static) for idx_UE_obj in range(NumPUE)) + \
            sum(power_transformS(model, idx_SUE_power)*mu + SUE_static for idx_SUE_power in range(NumSUE)) + PT_static + NumRISEle*RIS_static
    ...
    def power_transformS(model, idx_power):
        
        return pow(10, (model.power_S[idx_power]/10))/pow(10,3)
    
    def interference_index(model, idx_user, idx_user_I):
        
        real_idx_user = 0
        if (idx_user_I == idx_user):
            real_idx_user = 0
        else:
            real_idx_user = 1
        
        return real_idx_user
    ...
    def constraints_04(model, idx_qos_p):
        ...
        return (log(1 + ((power_transform(model, idx_qos_p) * Channel_square(model, 0, idx_qos_p)) / throughput_PUE_interference(model, idx_qos_p))) / log(2) ) >= P_Rmin
    
    def constraints_05(model, idx_qos_s):
        
        # return t_b*Bandwidth*(log(1+((power_quantized(model, idx_cons) * RIS_square(model, idx_cons)) /sigma2)) / log(2)) >= LocalDataSize/T_e
        # return Bandwidth * (log(1+((PUE_power[idx_pqos] * RIS_square_PUE(model, 0, idx_pqos)) /(interference_PUE(model, idx_pqos) + sigma2))) / log(2)) >= Bandwidth * (log(sqrt(2))/log(2)) 
        return (log(1 + ((power_transformS(model, idx_qos_s) * Channel_square(model, 1, idx_qos_s)) / throughput_SUE_interference(model, idx_qos_s))) / log(2) ) >= S_Rmin 
    
    def constraints_06(model, idx_pcons_S):
        
        # return t_b*Bandwidth*(log(1+((power_quantized(model, idx_cons) * RIS_square(model, idx_cons)) /sigma2)) / log(2)) >= LocalDataSize/T_e
        return model.power_S[idx_pcons_S] >= 0
    
    model = ConcreteModel(name="Symbiotic_Radio_BB_test")
    
    model.power = Var([i_PUE for i_PUE in range(NumPUE)], bounds=(1,P_max), within=Integers, initialize = BB_power_ini)
    model.power_S = Var([i_SUE for i_SUE in range(NumSUE)], bounds=(1,P_max), within=Integers, initialize = BB_power_iniS)
    ...
    for idx_cons5 in range(NumSUE):
        model.cons.add(constraints_05(model, idx_cons5))
    
    for idx_cons6 in range(NumSUE):
        model.cons.add(constraints_06(model, idx_cons6)) 
    
    model.obj = Objective(expr=obj_function_SE(model), sense=maximize)  #-----------------setting objective function-------------------#
    ...
    model_Spower_BB = np.array(list(model.power_S.get_values().values()))
    ...
    for idx_t in range(NumSUE):
        model_Spower_BB[idx_t] = pow(10, (model_Spower_BB[idx_t]/10))/pow(10,3)
    
    # model_onoff_BB = np.array(list(model.xnf.get_values().values()))
    # # model_onoff_BB = np.array([0,1])
    
    RIS_I_BB_status = results.solver.termination_condition
    RIS_I_BB_objective = model.obj()
    ...
    # Interference_Primary = sum(model_Spower_BB[idx_ips] * (Diag_square_P[idx_ips]) for idx_ips in range NumSUE)
    def Interference_index(index_user, index_iru, NumberUser):
        
        IR_U = 0
        if index_iru == index_user:
            IR_U = 0
        else:
            IR_U = 1
    ...
    def Interference_Primary(index_pri_user):
        IR_primary = 0
        IR_primary = sum(Interference_index(index_pri_user, idx_ip, NumPUE) * model_power_BB[idx_ip] * Diag_square_P[index_pri_user] for idx_ip in range(NumPUE)) \
            + sum(model_Spower_BB[idx_ips] * (Diag_square_P[index_pri_user]) for idx_ips in range(NumSUE))
        
        return IR_primary
            
    def Interference_Secondary(index_sec_user):
        IR_secondary = 0
        IR_secondary = sum(model_power_BB[idx_isp] * Diag_square_S[index_sec_user] for idx_isp in range(NumPUE)) \
            + sum(Interference_index(index_sec_user, idx_irs, NumPUE) * model_Spower_BB[idx_irs] * (Diag_square_S[index_sec_user]) for idx_irs in range(NumSUE))
        
        return IR_secondary
    
    Throughput_P_sum = sum( ma.log2(1 + ( model_power_BB[idx_sum_p] * Diag_square_P[idx_sum_p] / 
                                    ( Interference_Primary(idx_sum_p) + sigma2)) )  for idx_sum_p in range(NumPUE) )
        
    Throughput_S_sum = sum( ( ma.log2(1 +  ( model_Spower_BB[idx_sum_s] * Diag_square_S[idx_sum_s] / 
                                            ( (Interference_Secondary(idx_sum_s)) + sigma2)) ) ) for idx_sum_s in range(NumSUE))
    
    Total_power = sum( model_power_BB[idx_power_p]*mu + PUE_static  for idx_power_p in range(NumPUE) ) + \
                    sum( model_Spower_BB[idx_power_s]*mu + SUE_static  for idx_power_s in range(NumSUE) ) + PT_static + NumRISEle*RIS_static
    ...
    Obj_veri_SE = (Throughput_P_sum + Throughput_S_sum)
    ...
    print("DQN BB status: ", RIS_I_BB_status)
    print("Power consume : ", sum_power, ":W")
    print("Solver Obj SERIS: ", RIS_I_BB_objective)
    # print("*************************************************************************")
    ...
    print("obj veri SERIS: ", Obj_veri_SE)
    # print("Interference PUE: ", Interference_PUE)
    EE_RIS_IR_time = results.solver.time
    
    
    # return 0
    return RIS_I_BB_status, RIS_I_BB_objective, model_power_BB, model_theta_BB, EE_RIS_IR_time
