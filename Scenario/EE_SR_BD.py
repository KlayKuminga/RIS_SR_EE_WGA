...
def EE_SRBD(NumBSAnt, NumRISEle, NumUE, NumPUE, NumSUE, RISgroup, Bandwidth, sigma2, L_spreadingfactor,
    ...
    def BD_channel_square(model, idx_group, idx_square_UE):
        ...
        return sum((H_U2B_RicRe[idx_group][idx_square_UE][idx_BS].real + model.reflect * U_BD_B_Channel[idx_group][idx_square_UE][idx_BS].real)**2 +\
                    (H_U2B_RicRe[idx_group][idx_square_UE][idx_BS].imag + model.reflect * U_BD_B_Channel[idx_group][idx_square_UE][idx_BS].imag)**2 for idx_BS in range(0, NumBSAnt))
    ...
    model.reflect = Var([i for i in range(NumRISEle)], bounds=(0, 1), within=Reals, initialize = BB_theta_ini)
    ...
    print("Solver Obj EEBD: ", SymbioticBD_BB_objective)
    # print("*************************************************************************")
